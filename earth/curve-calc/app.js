...
(function(){
  var FEETPERMETRE, MILESPERKM, EARTHRADIUS, RADIANSPERKM, UNITS, units;
  window.log = function(){
  ...
  function calculate(){
    var h0, d0, h0_m, d0_km, d1_km, h1_m, d1, h1;
    h0 = getVal('h0');
    d0 = getVal('d0');
    h0_m = h0 * units.minor.factor;
    d0_km = d0 * units.major.factor;
    d1_km = getHorizonDistance_km(h0_m);
    h1_m = getTargetHiddenHeight_m(d0_km, d1_km);
    d1 = d1_km / units.major.factor;
    h1 = h1_m / units.minor.factor;
    $('#d1').text(d1);
    return $('#h1').text(h1);
  }
  function getHorizonDistance_km(h0_m){
    var rVert, theta;
    rVert = EARTHRADIUS - h0_m * 0.001;
    theta = Math.acos(rVert / EARTHRADIUS);
    return theta / RADIANSPERKM;
  }
  function getTargetHiddenHeight_m(d0_km, d1_km){
    var rads, rVert;
    if (d0_km < d1_km) {
      return 0;
    }
    rads = (d1_km - d0_km) * RADIANSPERKM;
    rVert = EARTHRADIUS * Math.cos(rads);
    return (EARTHRADIUS - rVert) * 1000;
  }
  function getVal(it){
